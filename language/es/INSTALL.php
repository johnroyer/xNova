<?php

$LNG['continue']			= "Continuar";

$LNG['menu_intro']			= "Intro";
$LNG['menu_install']		= "Instalar";
$LNG['menu_license']		= "Licencia";

$LNG['intro_lang']			= "Idioma";
$LNG['intro_instal']		= "Instalaci&oacute;n";
$LNG['intro_welcome']		= "Bienvenido a SpaceOne";
$LNG['intro_text']			= "SpaceOne es un juego de estrategia en el espacio, trabajado sobre la version 1.3 de 2moons, el proyecto cuenta con un <a href=\"http://spaceone.tk/foro\" target=\"_blank\">foro</a> de soporte oficial en el cual podran encontrar mayor informaci&oacute;n.<br /><br />El sistema de instalaci&oacute;n le guiar&aacute; a trav&eacute;s de la instalaci&oacute;n.<br>SpaceOne es un proyecto de c&oacute;digo abierto bajo la licencia GNU GPL v3. Para la licencia por favor haga clic en el elemento correspondiente localizado en el men&uacute;.<br><br>Antes de la instalaci&oacute;n, se comprueban las caracteristicas del servidor, si se cumplen los requisitos del sistema continuar&aacute; la instalaci&oacute;n.<br><br><br><center><font color=\"red\">Se recomienda tener conocimientos en Php para el autosoporte del servidor.</font></center>";

$LNG['reg_yes']			    = "S&iacute;";
$LNG['reg_no']				= "No";
$LNG['reg_found']			= "Encontrado";
$LNG['reg_not_found']		= "No encontrado";
$LNG['reg_writable']		= "Escritura";
$LNG['reg_not_writable']	= "No escribible";
$LNG['reg_file']			= "Archivo";
$LNG['reg_dir']			    = "Carpeta";
$LNG['reg_gd_need']		    = "GB-Lib disponible:";
$LNG['reg_bcmath_need']		= "BCMath disponible:";
$LNG['reg_iniset_need']		= "ini_set() disponible:";
$LNG['reg_mysqli_active']       = "MySQLi disponible:";
$LNG['reg_json_need']		= "JSON disponible:";
$LNG['req_php_need']		= "PHP-Version (min. 5.2.5)";

$LNG['step1_mysql_server']	= "MySQL-DB-Server: <br>Estandar: localhost";
$LNG['step1_mysql_port']	= "MySQL-DB-Server-Port: <br>Estandar: 3306";
$LNG['step1_mysql_dbname']	= "MySQL-DB-Name: <br> Ej.: Game";
$LNG['step1_mysql_dbuser']	= "MySQL-DB-User: <br> Ej.: root";
$LNG['step1_mysql_dbpass']	= "MySQL-DB-Password: <br> Ej.: ar2r3c1";
$LNG['step1_mysql_prefix']	= "MySQL-DB-Prefix: <br> Ej.: uni1_";

$LNG['step2_db_error']		= "Error al crear la tabla de base de datos(s): %s";
$LNG['step2_db_con_fail']	= "No hay conexi&oacute;n con la base de datos.<br> %s";
$LNG['step2_conf_op_fail']	= "El fichero config.php no est� en modo CHMOD 777.";
$LNG['step2_db_connet_ok']	= "Conexi�n establecida satisfactoriamente con la Base de datos...";
$LNG['step2_db_create_ok']	= "Tablas creadas con �xito...";
$LNG['step2_conf_create']	= "Fichero config.php creadas �xito...";

$LNG['step3_create_admin']	= "Creaci&oacute;n de cuenta de administrador";
$LNG['step3_admin_name']	= "Administrador Usuario:";
$LNG['step3_admin_pass']	= "Administrador Contrase&ntilde;a:";
$LNG['step3_admin_mail']	= "Administrador E-Mail:";


$LNG['step4_need_fields']	= "Tienes que llenar todos los campos.";

?>
